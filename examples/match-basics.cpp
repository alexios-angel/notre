// Basic matching: notre::match, notre::search, notre::starts_with.
//
...
static_assert(notre::match<"[a-z]+[0-9]+">("hello42"sv));
static_assert(!notre::match<"[a-z]+">("hello42"sv));
static_assert(notre::search<"[0-9]+">("hello42"sv));
static_assert(notre::starts_with<"[a-z]+">("hello42"sv));

// the functions are functors, so they can be stored and passed around
constexpr auto is_identifier = notre::match<"[a-zA-Z_][a-zA-Z0-9_]*">;

int main() {
	std::cout << std::boolalpha;
	std::cout << "match [a-z]+[0-9]+ on 'hello42':  " << bool(notre::match<"[a-z]+[0-9]+">("hello42"sv)) << "\n";
	std::cout << "search [0-9]+ on 'hello42':       " << bool(notre::search<"[0-9]+">("hello42"sv)) << "\n";
	std::cout << "starts_with [a-z]+ on 'hello42':  " << bool(notre::starts_with<"[a-z]+">("hello42"sv)) << "\n";
	...
	if (auto m = notre::search<"[0-9]+">("port is 8080, ok?"sv)) {
	...
	std::cout << "with a comment:                   "
	          << bool(notre::match<"ab(?#matches nothing, documents plenty)c">("abc"sv)) << "\n";
	std::cout << "\\o{101} is 'A':                   " << bool(notre::match<"\\o{101}">("A"sv)) << "\n";
	std::cout << "\\cI is a tab:                     " << bool(notre::match<"a\\cIb">("a\tb"sv)) << "\n";
}
